...
void EthGetworkClient::begin_connect()
{
    if (!m_endpoints.empty())
    {
        ...
        m_socket.async_connect(
            m_endpoint, m_io_strand.wrap(boost::bind(&EthGetworkClient::handle_connect, this,
...
void EthGetworkClient::processResponse(Json::Value& JRes) 
{
    ...
    else if (_id >= 40 && _id <= m_solution_submitted_max_id)
    {
        ...
        chrono::milliseconds _delay = chrono::duration_cast<chrono::milliseconds>(
            chrono::steady_clock::now() - m_pending_tstamp);
...
{
    send(string(Json::writeString(m_jSwBuilder, jReq)));
}
...
void EthGetworkClient::getwork_timer_elapsed(const boost::system::error_code& ec) 
{
    ...
    if (!ec)
    {
        ...
        chrono::seconds _delay =
            chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() - m_current_tstamp);
